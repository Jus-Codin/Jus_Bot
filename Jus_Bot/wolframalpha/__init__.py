"""
A modification of jaraco's Wolfram|Alpha v2.0 API wrapper to make it asynchronous
"""
...
class Client:
  ...
  async def query(self, input, params=(), **kwargs):
    ...
    >>> res = await client.query(input='pi', assumption='*C.pi-_*NamedConstant-')
    ...
    async with ClientSession() as session:
      async with session.get(url) as resp:
        ...
        doc = xmltodict.parse(await resp.text("utf-8"), postprocessor=Document.make)
